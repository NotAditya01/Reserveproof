...
export class EncryptionService {
    async encrypt(plaintext, secretKey) {
        ...
        const key = await pbkdf2(secretKey, salt, PBKDF2_ITERATIONS, KEY_LENGTH_BYTES, KEY_DERIVATION_ALGORITHM);
    ...
    async decrypt(payload, secretKey) {
        ...
        const key = await pbkdf2(secretKey, salt, // Use the *exact same salt* from the payload
        PBKDF2_ITERATIONS, KEY_LENGTH_BYTES, KEY_DERIVATION_ALGORITHM);
