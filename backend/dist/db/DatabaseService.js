...
export class DatabaseService {
    ...
    async createUser(email, hashedPsw, encryptedSeed, encryptedState, encryptedSK) {
        ...
        try {
            ...
            if (error.code === '23505') { // Error code 23505 is for unique object exists in db
    ...
    async getReserveAttestationByIdAndWallet(attestationId, walletAddress) {
        const query = `
    ...
    async getReserveByProofHash(proofHash) {
        const query = `
        ...
        return {
            ...
            overallVerified: typeof result.rows[0].overall_verified === 'boolean'
    ...
    async getReserveHistoryByWallet(walletAddress) {
        const query = `
    ...
    async getReserveHistoryByProtocol(protocolName) {
        const query = `
