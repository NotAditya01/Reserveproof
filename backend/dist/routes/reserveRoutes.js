...
function computeReserveRatio(totalReserves, totalLiabilities) {
    return Number(((totalReserves / totalLiabilities) * 100).toFixed(2));
}
function statusFromReserveRatio(reserveRatio) {
    if (reserveRatio >= 120)
        return 'SOLVENT';
    if (reserveRatio >= 100)
        return 'WARNING';
...
function reserveBandFromStatus(solvencyStatus) {
    if (solvencyStatus === 'SOLVENT')
        return 'Above 120%';
    if (solvencyStatus === 'WARNING')
        return '100–120%';
    return 'Below 100%';
}
function thresholdFromStatus(solvencyStatus) {
    if (solvencyStatus === 'SOLVENT')
        return 740;
    if (solvencyStatus === 'WARNING')
        return 580;
...
function scoreFromReserveRatio(reserveRatio) {
...
function requiredAttributes(category) {
    if (category === 'exchange')
        return new Set(['reserve_ratio']);
    if (category === 'defi')
        return new Set(['reserve_ratio']);
    if (category === 'nft')
        return new Set(['treasury_funded']);
    if (category === 'airdrop')
        return new Set(['treasury_solvent']);
    if (category === 'dao')
        return new Set(['treasury_solvency']);
    return new Set(['reserve_ratio', 'collateral_ratio']);
}
function evaluateAttribute(type, inputs, reserveRatio) {
    const labels = {
...
reserveRouter.post('/attest', async (req, res) => {
    const { walletAddress, protocolName, totalReserves, totalLiabilities, categoryType, attributes, } = req.body;
    ...
    let attributesSelected = null;
    let overallVerified = null;
...
reserveRouter.get('/feed', async (_req, res) => {
