...
export async function createMidnightWallet(seed) {
...
export function signTransactionIntents(tx, signFn, proofMarker) {
    ...
    for (const segment of tx.intents.keys()) {
        ...
        if (cloned.fallibleUnshieldedOffer) {
            const sigs = cloned.fallibleUnshieldedOffer.inputs.map((_, i) => cloned.fallibleUnshieldedOffer.signatures[i] ?? signature);
            cloned.fallibleUnshieldedOffer =
                cloned.fallibleUnshieldedOffer.addSignatures(sigs);
        }
        if (cloned.guaranteedUnshieldedOffer) {
            const sigs = cloned.guaranteedUnshieldedOffer.inputs.map((_, i) => cloned.guaranteedUnshieldedOffer.signatures[i] ?? signature);
...
export async function createProviders(walletCtx) {
    const state = await Rx.firstValueFrom(walletCtx.wallet.state().pipe(Rx.throttleTime(3000), Rx.filter((s) => s.unshielded?.progress?.isStrictlyComplete?.() === true)));
...
//# sourceMappingURL=midnight-utils.js.map