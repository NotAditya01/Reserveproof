...
function getOrCreatePrivateStatePassword() {
...
async function main() {
    ...
    const { createMidnightWallet, createProviders, loadContractModule, zkConfigPath, } = await import('./services/midnight-utils.js');
    ...
    if (!seed) {
        throw new Error('BACKEND_WALLET_SEED is required and must be a 24-word mnemonic phrase from Lace');
    }
    ...
    const state = await Rx.firstValueFrom(walletCtx.wallet.state().pipe(Rx.throttleTime(3000), Rx.tap(() => process.stdout.write('.')), Rx.filter((s) => s.unshielded?.progress?.isStrictlyComplete?.() === true)));
    console.log('\n  ✅ Wallet synced!\n');
    ...
    if (balance === 0n) {
        ...
        await Rx.firstValueFrom(walletCtx.wallet.state().pipe(Rx.throttleTime(15000), Rx.tap(() => process.stdout.write('.')), Rx.filter((s) => {
    ...
    const dustState = await Rx.firstValueFrom(walletCtx.wallet.state().pipe(Rx.throttleTime(3000), Rx.filter((s) => s.isSynced === true)));
    if (dustState.dust?.walletBalance?.(new Date()) === 0n) {
        ...
        const waitForDust = Rx.firstValueFrom(walletCtx.wallet.state().pipe(Rx.throttleTime(5000), Rx.filter((s) => s.unshielded?.progress?.isStrictlyComplete?.() === true), Rx.filter((s) => (s.dust?.walletBalance?.(new Date()) ?? 0n) > 0n)));
        const timeout = new Promise((_, reject) => {
            setTimeout(() => {
                reject(new Error(`Timed out waiting for DUST after ${timeoutMs}ms. ` +
    ...
    const compiledContract = CompiledContract.make('ep-contract', ContractModule.Contract).pipe(CompiledContract.withWitnesses(deployWitnesses), CompiledContract.withCompiledFileAssets(zkConfigPath));
